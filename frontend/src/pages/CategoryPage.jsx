...
const FALLBACKS = {
  ...
  grocery: 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=400&h=300&fit=crop&q=80',
  pharmacy: 'https://images.unsplash.com/photo-1631549916768-4119b2e5f926?w=400&h=300&fit=crop&q=80',
};
...
const categoryConfig = {
  ...
  groceries: {
    ...
    subcategories: [
      { name: 'All', icon: GroceryIcons.fruits },
...
const CategoryPage = () => {
  ...
  const fallbackImg = FALLBACKS[storeType] || FALLBACKS.restaurant;
  ...
  const StoreCardHorizontal = ({ restaurant }) => (
    <Link
    ...
    >
      <div className="relative h-28 md:h-32 w-full overflow-hidden">
        ...
        />
        <div className="absolute top-2 right-2 flex items-center gap-1 bg-white/90 text-blackc text-[10px] font-bold px-2 py-0.5 rounded-full">
      ...
      <div className="p-2.5">
        ...
        <p className="text-[11px] text-gray-500 line-clamp-1 mb-2">{restaurant.description || restaurant.cuisine_type || 'Fresh and tasty'}</p>
        <div className="flex items-center space-x-2 text-[11px] text-gray-600">
          <span className="flex items-center space-x-1">
            <img src="/Icons/Icon.png" alt="delivery" className="w-3.5 h-3.5" onError={(e) => e.target.style.display = 'none'} />
            <span className="font-semibold text-blackc">${parseFloat(restaurant.delivery_fee || 3).toFixed(2)}</span>
          </span>
          <span className="flex items-center space-x-1">
            <img src="/Icons/timer.png" alt="time" className="w-3.5 h-3.5" onError={(e) => e.target.style.display = 'none'} />
  ...
  const StoreCardList = ({ restaurant }) => (
    <Link
      to={`/restaurant/${restaurant.id}`}
    ...
    >
      ...
      <div className="flex-1 pl-3 min-w-0">
        ...
        <div className="flex items-center flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-gray-600">
          <span className="flex items-center space-x-1">
            <img src="/Icons/Icon.png" alt="delivery" className="w-3.5 h-3.5" onError={(e) => e.target.style.display = 'none'} />
            <span className="font-semibold text-blackc">${parseFloat(restaurant.delivery_fee || 3).toFixed(2)}</span>
          </span>
          ...
          <span className="flex items-center space-x-1">
            <img src="/Icons/star-Filled.png" alt="rating" className="w-3 h-3" onError={(e) => e.target.style.display = 'none'} />
            <span>{parseFloat(restaurant.rating || 4.5).toFixed(1)}</span>
          </span>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-grey-full-light pb-24">
      ...
      {config.bannerImage && (
        <div className="relative h-40 md:h-56 w-full overflow-hidden rounded-b-3xl -mt-6">
          <img
            ...
            onError={(e) => { if (config.bannerFallback) e.target.src = config.bannerFallback; }}
          />
      ...
      <div className={`p-4 ${config.bannerImage ? '-mt-8 relative z-10 rounded-t-3xl bg-grey-full-light' : ''}`}>
        ...
        <div className="flex items-center space-x-3 mb-5">
          <button
            onClick={() => navigate(-1)}
            className="w-9 h-9 bg-white border border-grey-light-dark rounded-full flex items-center justify-center text-blackc hover:bg-grey-light-dark transition-colors shadow-sm flex-shrink-0"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
          </button>
          <h1 className="text-xl font-bold text-blackc">{config.title}</h1>
          {!loading && !error && (
            <span className="ml-1 text-sm text-gray-400 font-normal">({filteredBySubcategory.length})</span>
          )}
        </div>
                  ...
                  >
                    ...
                    >
                      <div style={{ filter: isActive ? 'brightness(0) invert(1)' : 'none' }}>
                        {sub.icon ?? (sub.image
                          ? <img src={sub.image} alt={sub.name} className="w-9 h-9 object-contain" onError={(e) => e.target.style.display = 'none'} />
                          : null)}
                      </div>
                    </div>
        ...
        {error && (
          <div className="text-center py-12">
            ...
            <button onClick={fetchRestaurants} className="text-primary text-sm font-bold hover:underline">Try again</button>
          </div>
        )}
        ...
        {loading && (
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex gap-3 animate-pulse bg-white p-3 rounded-2xl border border-grey-light-dark">
                  <div className="w-24 h-20 bg-grey-light-dark rounded-xl flex-shrink-0"></div>
                  <div className="flex-1 space-y-2 py-1">
                    <div className="h-3 bg-grey-light-dark rounded w-2/3"></div>
                    <div className="h-2 bg-grey-light-dark rounded w-1/2"></div>
                    <div className="h-2 bg-grey-light-dark rounded w-1/3"></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <>
            {filteredBySubcategory.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="font-bold text-blackc mb-1">No stores found</p>
                <p className="text-gray-400 text-sm">
                  {selectedSubcategory !== 'All'
                    ? `No results in "${selectedSubcategory}". Try a different category.`
              ...
              <>
                ...
                <div className="flex justify-between items-center mb-3">
                  ...
                  <span className="text-sm font-semibold text-gray-800 px-4 py-1.5 rounded-full" style={{ backgroundColor: '#A1EEC7' }}>
                    {filteredBySubcategory.length} stores
                  </span>
                </div>

                <div className="flex space-x-4 overflow-x-auto no-scrollbar pb-4 mb-6">
                  {filteredBySubcategory.map((restaurant) => (
                    <StoreCardHorizontal key={restaurant.id} restaurant={restaurant} />
                  ))}
                </div>

                {/* Promotions list */}
                <div className="flex justify-between items-center mb-3">
                  <h2 className="text-lg font-bold text-blackc">Promotions</h2>
                  <span className="text-sm font-semibold text-gray-800 px-4 py-1.5 rounded-full" style={{ backgroundColor: '#A1EEC7' }}>
                    See all
                  </span>
                </div>
                ...
                <div className="space-y-3">
                  {filteredBySubcategory.map((restaurant) => (
                    <StoreCardList key={`list-${restaurant.id}`} restaurant={restaurant} />
                  ))}
                </div>
              </>
            )}
