...
const AdminDashboard = () => {
  ...
  const toggleCuisine = (cuisine) => {
    setStoreForm((prev) => {
      const current = Array.isArray(prev.cuisineType)
        ? prev.cuisineType
        : String(prev.cuisineType || '')
            .split(',')
            .map((s) => s.trim())
            .filter(Boolean);
  ...
  const handleStoreSubmit = async (e) => {
    ...
    try {
      if (editingStore) {
        ...
        await restaurantService.create(formToSend);
      ...
      console.error('Store submit error:', err.response?.data || err.message);
  ...
  const handleEditStore = (store) => {
    setStoreForm({
      ...
      cuisineType: store.cuisine_type
        ? store.cuisine_type.split(',').map((s) => s.trim()).filter(Boolean)
        : [],
  ...
  const handleDeleteStore = async (store) => {
    ...
    try {
      await restaurantService.remove(store.id);
      ...
      <div className="px-4 -mt-4 relative z-10">
        {success && (
          <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl mb-4 flex items-center justify-between shadow-sm">
            ...
            <button onClick={() => setSuccess(null)} className="text-green-400 hover:text-green-700 ml-3">
              ✕
            </button>
          </div>
        )}
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl mb-4 flex items-center justify-between shadow-sm">
            ...
            <button onClick={() => setError(null)} className="text-red-400 hover:text-red-700 ml-3">
              ✕
            </button>
          </div>
        )}
                  ...
                  <div>
                    ...
                    <div className="flex space-x-3">
                      {[
                        ...
                        const styles = storeTypeStyles[type.key];
                        const isSelected = storeForm.storeType === type.key;
                        ...
                        return (
                          <button
                            ...
                            className={`flex-1 py-3 px-3 rounded-xl text-sm font-bold border-2 transition-all ${
                              isSelected
                                ? styles.border
                      ...
                      <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                        ...
                        {Array.isArray(storeForm.cuisineType) && storeForm.cuisineType.length > 0 && (
                          <span className="ml-2 text-primary font-normal text-xs">
                            ({storeForm.cuisineType.length} selected)
                          </span>
                        )}
                      </label>

                      <div className="flex flex-wrap gap-2">
                        {(cuisineTypesByStore[storeForm.storeType] || []).map((cuisine) => {
                          const selected =
                            Array.isArray(storeForm.cuisineType) &&
                            storeForm.cuisineType.includes(cuisine);
              ...
              <div className="card p-5 mb-5 border-2 border-primary">
                ...
                <form onSubmit={handleMenuSubmit} className="space-y-4">
                  ...
                  <div className="flex space-x-3">
                    ...
                    <button type="button" onClick={() => setShowMenuForm(null)} className="btn-secondary px-6">
                      Cancel
                    </button>
                  </div>
                </form>
                      ...
                      <div className="flex items-start space-x-4">
                        <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-grey-light border border-grey-light-dark">
                          <img
                            ...
                            onError={(e) => {
                              e.target.src =
                                defaultImages[store.store_type] || defaultImages.restaurant;
                            }}
                          />
                        </div>
                ...
                {orders.map((order) => (
                  <div key={order.id} className="card p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <h3 className="font-bold text-blackc text-sm">
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500 mb-3">
                      ...
                      <span className="font-bold text-blackc text-sm flex-shrink-0">
